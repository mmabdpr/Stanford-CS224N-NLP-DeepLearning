...
def naiveSoftmaxLossAndGradient(
    ...
    y_hat = softmax(outsideVectors @ centerWordVec).reshape((-1, 1))
    centerWordVec = centerWordVec.reshape((-1, 1))
    minus_v_c = np.zeros(y_hat.shape); minus_v_c[outsideWordIdx] = -1

    loss = -np.log(y_hat[outsideWordIdx]).item()
    gradCenterVec = -outsideVectors[outsideWordIdx].reshape((-1, 1)) + outsideVectors.T @ y_hat
    gradOutsideVecs = (minus_v_c @ centerWordVec.T) + (y_hat @ centerWordVec.T)
...
def negSamplingLossAndGradient(
    ...
    u_x__v_c = (outsideVectors[indices] @ v_c).reshape(-1, 1)

    loss = (-np.log(sigmoid(u_x__v_c[0])) - np.sum(np.log(sigmoid(-u_x__v_c[1:])), axis=0)).item()

    gradCenterVec = -(1 - sigmoid(u_x__v_c[0])) * u_o + np.sum(((1 - sigmoid(-u_x__v_c[1:])).T @ outsideVectors[negSampleWordIndices]), axis=0).reshape(-1, 1)

    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[negSampleWordIndices] = (1 - sigmoid(-u_x__v_c[1:])) @ v_c.T
    gradOutsideVecs[outsideWordIdx] = -(1 - sigmoid(u_x__v_c[0])) @ v_c.T
    idx_count = np.bincount(indices)
    for idx in set(indices):
        gradOutsideVecs[idx] *= idx_count[idx]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for w_o in outsideWords:
        loss_w, gradCenterVec_w, gradOutsideVecs_w = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[w_o], outsideVectors, dataset)
        loss += loss_w
        gradCenterVecs[word2Ind[currentCenterWord]] += gradCenterVec_w[:, 0]
