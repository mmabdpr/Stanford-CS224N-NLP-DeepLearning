...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # truncate (4 to int(self.block_size*7/8))
        t = random.randint(4, int(self.block_size*7/8))
        doc = doc[:t]

        # [prefix] [masked_content] [suffix] (len(masked_content) == 1/4*len(document) on avg)
        mc_len = int(np.random.normal(1/4*len(doc), scale=1/5*len(doc)))
        mc_len = max(1, mc_len)
        mc_len = min(len(doc) - 2, mc_len)

        p = random.randint(1, len(doc) - mc_len - 1)

        prefix = doc[:p]
        masked_content = doc[p:p+mc_len]
        suffix = doc[p+mc_len:]

        # assert min(len(prefix), len(masked_content), len(suffix)) >= 1, (len(doc), len(prefix), len(masked_content), len(suffix))

        # masked_string = [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] [pads]
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        masked_string += self.PAD_CHAR * (self.block_size - len(masked_string))

        assert len(masked_string) == self.block_size

        # masked_string[:-1], masked_string[1:]
...
if __name__ == '__main__':
    ...
    # noinspection SpellCheckingInspection
    arg_p.add_argument('dataset_type', help="Type of dataset to sample from."
