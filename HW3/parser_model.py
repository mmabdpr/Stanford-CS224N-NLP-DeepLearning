...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings: np.ndarray, n_features: int = 36,
                 hidden_size: int = 200, n_classes: int = 3, dropout_prob: float = 0.5):
        ...
        self.embed_size: int = embeddings.shape[1]
        ...
        self.embed_to_hidden_weight = nn.Parameter(
            torch.zeros([self.n_features * self.embed_size, self.hidden_size], dtype=torch.float, requires_grad=True)
            .to(self.device))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)

        self.embed_to_hidden_bias = nn.Parameter(
            torch.zeros([self.hidden_size], dtype=torch.float, requires_grad=True)
            .to(self.device))
        nn.init.uniform_(self.embed_to_hidden_bias)

        self.dropout = nn.Dropout(self.dropout_prob).to(device=self.device)

        self.hidden_to_logits_weight = nn.Parameter(
            torch.zeros([self.hidden_size, self.n_classes], dtype=torch.float, requires_grad=True)
            .to(self.device))
        nn.init.xavier_uniform_(self.hidden_to_logits_weight)
        self.hidden_to_logits_bias = nn.Parameter(
            torch.zeros([self.n_classes], dtype=torch.float, requires_grad=True)
            .to(self.device))
    ...
    def embedding_lookup(self, w: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.index_select(self.embeddings, 0, torch.flatten(w.to(self.device))).view(w.shape[0], -1)
    ...
    def forward(self, w: torch.Tensor) -> torch.Tensor:
        ...
        @return logits: tensor of predictions (output after applying the layers of the network)
        ...
        x = self.embedding_lookup(w.to(self.device))
