...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab: Vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(
            input_size=embed_size, hidden_size=hidden_size, num_layers=self.num_layers_enc, bias=True, bidirectional=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        h0 = torch.zeros((2 * self.num_layers_enc, X.shape[1], self.hidden_size)).to(self.device)
        c0 = torch.zeros((2 * self.num_layers_enc, X.shape[1], self.hidden_size)).to(self.device)
        X = nn.utils.rnn.pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X, (h0, c0))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t = torch.nn.Softmax(dim=1)(e_t)
